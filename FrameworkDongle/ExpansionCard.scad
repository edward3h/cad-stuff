...
module expansion_card_base(open_end, make_printable, pcb_mount="boss") {
    ...
    extra = 0.1;
    outer = [base[0], base[1], base[2]];
    inner = [outer[0]-side_wall*2, outer[1]-side_wall*2, outer[2]-side_wall+extra];
    boss_radius = 2.3;
    ...
    difference() {
        ...
        difference() {
            ...
            notch_h = 3.8;
            // The main hollow
            translate([side_wall, open_end ? -extra : side_wall, side_wall]) cube([inner[0], open_end ? inner[1]+side_wall+extra : inner[1], inner[2]]);
        ...
        edge_r = 0.8;
        // fillet(edge_r, outer[0]);
        
        // The USB-C plug cutout
        translate([outer[0]/2, outer[1], usb_c_r+usb_c_h]) usb_c_cutout(false);
        
        // The sliding rails
        translate([0, outer[1], rail_h]) rail(make_printable);
        translate([outer[0], outer[1], rail_h]) mirror([1, 0, 0]) rail(make_printable);
        ...
        translate([0, outer[1]-ledge_cut_d, 0]) cube([outer[0], ledge_cut_d, ledge_cut]);
        // The fillet on that cover
        translate([outer[0], outer[1]-ledge_cut_d, 0]) rotate([0, 0, 180]) fillet(ledge_cut/2, outer[0]);
    }
    ...
    if (pcb_mount == "boss" || pcb_mount == "boss_insert") {        
        ...
        translate([outer[0]-boss_x, boss_y, 0]) boss(boss_radius, pcb_mount == "boss_insert", make_printable);
        ...
        translate([outer[0]-boss_x-boss_radius, boss_y-boss_radius, 0]) {
            cube([boss_radius*2, boss_radius*2, pcb_h]);
            if (make_printable)
                translate([boss_radius, boss_radius*2, 0]) rib(boss_radius*2, pcb_h);
...
module extension() {    
    ...
    difference() {
        linear_extrude(height = base[2]) polygon(points=[[indent,0],[base[0]-indent,0],[base[0],extend],[0,extend]]);         
        translate([0,extra+side_wall,side_wall]) 
        linear_extrude(height = base[2]) 
        polygon(points=[[side_wall+indent,0],[base[0]-side_wall-indent,0],[base[0]-side_wall,extend-side_wall],[side_wall,extend-side_wall]]); 
    }
}

module inner_rib() {
    usb_indent=[1+side_wall,17,base[2]];
    union() {
        cube([side_wall,base[1]+extend-extra,base[2]]);
        translate([0,base[1]+extend-usb_indent[1]-extra,0]) cube(usb_indent);
    }
}

// Rotate into a printable orientation
rotate([-90, 0, 0]) translate([0, -base[1], 0]) union() {
    expansion_card_base(open_end = true, make_printable = true, pcb_mount="");
    translate([0,-(extend-0.1),0]) extension();
    translate([indent,-(extend-extra),0]) inner_rib();
    translate([base[0]-indent,-(extend-extra),0]) mirror([1,0,0]) inner_rib();
}